...
class PayrollRunReport(models.AbstractModel):
    _name = 'report.sme_hr.xlsx_payroll_run_report'
    _description = 'Payroll Run Report Xlsx'
            ...
            for slip in lines.slip_ids:
                if lines.slip_ids:
                    if slip.struct_id.id == used_struct[0]:
                        has_payslips = True
                        job_grade_value = dict(slip.employee_id._fields['x_studio_field_8cPqb'].selection).get(
                            slip.employee_id.x_studio_field_8cPqb)
                        ...
                        sheet.write(e_name, 4, str(slip.contract_id.x_studio_contractec_start_date.strftime(
                            "%d-%m-%Y")) if slip.contract_id.x_studio_contractec_start_date else '', format3)
                        sheet.write(e_name, 5,
                                    slip.contract_id.x_studio_anada_account_number.acc_number if slip.contract_id.x_studio_anada_account_number else '',
                                    format3)
                        ...
                        sheet.write(e_name, 9,
                                    slip.employee_id.bank_account_id.acc_number if slip.employee_id.bank_account_id else '',
                                    format3)
                        sheet.write(e_name, 10,
                                    slip.employee_id.x_studio_current_account_number.acc_number if slip.employee_id.x_studio_current_account_number else '',
                                    format3)
