...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        if ctx.get('account_ids'):
            ...
            arg_list += (tuple(ctx['account_ids'] or [0]),)
        ...
        for i in range(5):
            ...
            additional_clause = '' # myh@smeintellect.com
            args_list = (tuple(move_state), tuple(account_type), tuple(partner_ids),)

            if ctx.get('account_ids'):
                additional_clause += 'AND (account_account.id IN %s)'
                args_list += (tuple(ctx['account_ids']),)
            ...
            query = '''SELECT l.id
                       ...
                       WHERE (l.account_id = account_account.id) AND (l.move_id = am.id)
                           AND (am.state IN %s)
                           AND (account_account.internal_type IN %s)
                           AND ((l.partner_id IN %s) OR (l.partner_id IS NULL)) '''+ additional_clause +'''
                           AND ''' + dates_query + '''
                       AND (l.date <= %s)
                       AND l.company_id IN %s
        ...
        undue_amounts = {}
        additional_clause = '' # myh@sme.intellect.com
        ...
        query = '''SELECT l.id
                   ...
                   AND (l.date <= %s)
                   AND l.company_id IN %s '''+ additional_clause +'''
                   ORDER BY COALESCE(l.date_maturity, l.date)'''
        cr.execute(query,arg_list)
        aml_ids = cr.fetchall()
        aml_ids = aml_ids and [x[0] for x in aml_ids] or []
        ...
        for partner in partners:
            ...
            if partner['partner_id']:
                ...
                values['trust'] = False
            check_key = False
            if partner['partner_id'] in lines.keys() and lines[partner['partner_id']]:
                check_key = True

            if at_least_one_amount or (self._context.get('include_nullified_amount') and check_key ):
                res.append(values)
